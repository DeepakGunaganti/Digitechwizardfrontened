...
const ClientMasterDetails = () => {
  const [meta, setmeta] = useState([])
  const userdata = async () => {
    const data =await axios.get("https://digitechwizardbackend.onrender.com/api/getClientMaster")
    ...
    <div style={{ display: 'flex'  }} >
      ...
      <div style={{ marginLeft: '20%', padding: '20px', width: 'calc(100% - 250px)' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '20px' }}>
          <thead>
            <tr>
              {['#', 'Code', 'Name', 'Agent', 'Contact', 'D.O.J', 'P.W.D', 'Expo','Client Comm % Type', 'Match', 'SSN', 'Chips', 'Status'].map(header => (
          ...
          <tbody>
            {filterdata.length > 0 ? filterdata.map((item, index)=>{
               ...
               <td style={{ border: '1px solid #ddd', padding: '8px' }}>{item?.Agent}</td>
               ...
               <td style={{ border: '1px solid #ddd', padding: '8px' }}>{item?.Expo}</td>
               <td style={{ border: '1px solid #ddd', padding: '8px' }}>{item?.ClientComm}</td>
...
export default ClientMasterDetails;
